...
//   //   useEffect(() => {
//   //     socketRef.current = io("ws://192.168.1.26:3500");
...
//         "http://192.168.1.26:3500/api/auth/login",
//         body,
//         {
//           headers: {
  ...
  const getAllusers = async () => {
    try {
      const response = await axios.get(
        "http://192.168.1.26:3500/api/auth/getAll"
      );
  ...
  const onNavigate = async (sender, reciver) => {
    try {
      ...
      const createdId = await axios.post(
        "http://192.168.1.26:3500/api/room/createRoom",
        body
      );
